...
abstract class TushareRequest {
    ...
    inline fun <reified I, O> invoke(inParams: I, outParamsClass: Class<O>, fields: Array<String> = arrayOf()): Array<O> {
        val responseBody = tushareRequestBodyFactory.tushareRequestBody(apiName, inParams, fields).let(tushareFeignClient::request)
        if (responseBody.code == 0)
            return objectMapper.convertValue(responseBody.data, object : TypeReference<Array<O>>() {})
        else
