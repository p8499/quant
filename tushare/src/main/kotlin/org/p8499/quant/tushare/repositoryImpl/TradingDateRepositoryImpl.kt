...
class TradingDateRepositoryImpl : TradingDateDao {
    ...
    override fun vacantForLevel1Candlestick(stockId: String): List<TradingDate> = em
            .createQuery("select t0 from TradingDate as t0 left join Stock as t1 on t1.exchangeId = t0.exchangeId where t1.id = :stockId and not exists (select 1 from Level1Candlestick as t2 where t2.stockId = :stockId and t2.date = t0.date)", TradingDate::class.java)
            .setParameter("stockId", stockId).resultList
    ...
    override fun vacantForLevel1Basic(stockId: String): List<TradingDate> = em
            .createQuery("select t0 from TradingDate as t0 left join Stock as t1 on t1.exchangeId = t0.exchangeId where t1.id = :stockId and not exists (select 1 from Level1Basic as t2 where t2.stockId = :stockId and t2.date = t0.date)", TradingDate::class.java)
            .setParameter("stockId", stockId).resultList
    ...
    override fun vacantForLevel1AdjFactor(stockId: String): List<TradingDate> = em
            .createQuery("select t0 from TradingDate as t0 left join Stock as t1 on t1.exchangeId = t0.exchangeId where t1.id = :stockId and not exists (select 1 from Level1AdjFactor as t2 where t2.stockId = :stockId and t2.date = t0.date)", TradingDate::class.java)
            .setParameter("stockId", stockId).resultList
    ...
    override fun vacantForLevel2(stockId: String): List<TradingDate> = em
            .createQuery("select t0 from TradingDate as t0 left join Stock as t1 on t1.exchangeId = t0.exchangeId where t1.id = :stockId and not exists (select 1 from Level2 as t2 where t2.stockId = :stockId and t2.date = t0.date)", TradingDate::class.java)
            .setParameter("stockId", stockId).resultList
