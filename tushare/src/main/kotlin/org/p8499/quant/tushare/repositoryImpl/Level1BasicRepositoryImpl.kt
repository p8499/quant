...
class Level1BasicRepositoryImpl : Level1BasicDao {
    ...
    override fun get(stockId: String, date: Date): Level1Basic? = em
            .createQuery("select t0 from Level1Basic as t0 where t0.stockId = :stockId and t0.date <= :date and not exists (select 1 from Level1Basic t1 where t1.stockId = :stockId and t1.date > t0.date)", Level1Basic::class.java)
